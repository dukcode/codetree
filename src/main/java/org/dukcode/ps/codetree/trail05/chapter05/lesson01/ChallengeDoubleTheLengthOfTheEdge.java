...
/**
 * @see <a
 * href="https://www.codetree.ai/ko/trails/complete/curated-cards/challenge-double-the-length-of-the-edge/description">LINK</a>
 */
public class ChallengeDoubleTheLengthOfTheEdge {
  ...
  private static int solve() {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        ...
        adj[i][j] *= 2;
        adj[j][i] *= 2;
        int distDouble = dijkstra(0)[n - 1];
        ans = Math.max(ans, distDouble - distFromStart[n - 1]);
        adj[i][j] /= 2;
        adj[j][i] /= 2;
      }
    }
